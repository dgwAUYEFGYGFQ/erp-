  ...
  const PrototypeLayout = {
    ...
    template: `
      <header class="topbar">
        ...
        <div class="topbar-center procurement-topbar-center">
          <nav class="procurement-top-nav" aria-label="采购云主导航">
            <a v-for="(item, index) in topMenus" :key="item" href="javascript:void(0)" class="procurement-top-nav-item" :class="{ 'is-active': item === '新能源订单管理' }">{{ item }}</a>
            <a href="javascript:void(0)" class="procurement-top-nav-item top-nav-more"><i class="ri-more-fill"></i></a>
          </nav>
        </div>
        ...
        <div class="content-wrap">
          <div v-if="openTabs.length" class="opened-page-tabs-bar" data-tour="opened-page-tabs">
            <div class="opened-page-tabs-scroll">
              <div class="opened-page-tab-shell procurement-workbench-tab">
                <button type="button" class="opened-page-tab"><span class="opened-page-tab-text">工作台</span></button>
              </div>
              <el-tooltip
